...
<div class="row">
    ...
    <!-- ici form ajouter sous sous categorie -->  
    <form action="ar_s_scategorie" method="post" enctype="multipart/form-data">
        {!! csrf_field() !!}
        <div class="col-md-6">
            <div class="panel panel-primary">
                ...
                <div class="panel-body">
                    <div class="form-group">
                        <label>
                          ادخل اسم الفئة الجزئية
                        </label>
                        <div class="input-group">
                            <div class="input-group-addon">
                                <i class="fa fa-edit"></i>
                            </div>
                            <input type="text" name="NomSousScatg" class="form-control"  data-mask/>
                        </div>
                    </div>
                     ...
                     <div class="col-md-12">
                                            <div class="form-group">
                                                <label for="e1" class="control-label">  اختر الفئةالفرعية </label>
                                                <select name="idsouscatg" id="e1" class="form-control select2">
                                                    @foreach($categories as $key=>$categorie)
                                                    <optgroup label="{{ $categorie ->name }}">
                                                         @foreach($souscategories as $key=>$souscategorie)
                                                           @if( $souscategorie ->idcatg == $categorie->id)
                    ...
                    <div class="col-xs-6 col-md-6">
                        <input type="submit" class="btn btn-primary btn-block btn-md btn-responsive"  value="تسجيل">
                    </div>
    ...
    <!-- end form ajouter sous sous categorie -->
                ...
                <div class="x_panel">
                  ...
                  <div class="x_content">
                    ...
                    <table id="datatable" class="table table-striped table-bordered">
                      ...
                      <tbody>
                            @foreach($s_scategories as $key=>$s_scategorie)
                                ...
                                <form action="ar_s_scategorie/{{$s_scategorie ->id}}" method="POST" >
                                    {!! csrf_field() !!}
                                     <input type="hidden" name="_method" value="PATCH"/>
                                     @foreach($souscategories as $key=>$scat)
                                     @if( $scat->id == $s_scategorie ->id_souscatg)
                                     
                                    
                                    
                                      @foreach($categories as $ke=>$cat)
                                     @if( $cat->id ==$scat->idcatg)
                                     <td >
                                        {{ $cat->name }}
                                    </td>             
                                     @endif
                                    ...
                                    <td >
                                        <input type="text" name="name_sscatg" class="form-control input-small" value="{{ $s_scategorie ->name }}">
                                    </td>
